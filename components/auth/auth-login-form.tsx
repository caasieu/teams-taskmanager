...
export function AuthLoginForm() {
  ...
  async function onSubmit(data: LoginFormData) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(result.error || "Invalid credential pairing matching profile records.");
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="w-full">
      <div className="flex flex-col gap-2">
        ...
        {errorMessage && (
          <div className="p-2.5 mb-1 text-[11px] font-mono font-medium border border-red-200 text-red-600 bg-red-50/50 rounded-sm leading-relaxed animate-shake">
            ⚠️ {errorMessage}
          </div>
        )}
        ...
        <AuthSubmitButton 
          label={isSubmitting ? "Authenticating session..." : "Let's get you in!"} 
        />
      </div>
    </form>
  );
